...
```{r}
p1 <- plotDR(sce, color_by = "meta10")
p1
```

## before annotation, we will briefly assess if treatment/time affect cluster proportions
...
## plot proportions
```{r}
proportions <- neut.data %>% group_by(k10, treatment, time, donor) %>% dplyr::count(k10, treatment, time, donor)
...
p1 <- ggplot(proportions, aes(x = time, y = number, fill = cluster))+geom_col(position = "dodge")+theme_bw()+
  scale_fill_manual(values = )+facet_wrap(~treatment, scales = "free_x", ncol = 4)+
